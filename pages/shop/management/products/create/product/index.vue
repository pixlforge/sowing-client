<template>
  <div class="container">
    ...
    <!-- Form -->
    <form class="flex flex-wrap -mx-10" @submit.prevent="store">
      <!-- French -->
      <FormSection>
        <FormSectionTitle>
          {{ $t('languages.french') }}
...
export default {
  ...
  head() {
    return {
      title: `${this.$t('product.management.create.title')} | ${
        this.shop.name
      }`,
      meta: [
        {
  ...
  methods: {
    ...
    async store() {
      try {
        this.errors = {}
        const product = await this.$axios.$post('/products', this.form)
        this.$router.push({
