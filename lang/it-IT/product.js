export default {
  product: {
    img: {
      alt: 'Immagine del prodotto',
    },
    creator: {
      category: {
        category: 'Categoria',
      },
      infotip: {
        add_type: 'Iniziate aggiungendo un tipo',
        add_variation:
          'Quindi aggiungete una variante del vostro prodotto base',
        add_many: 'Aggiungete tutti i tipi e le varianti che desiderete',
      },
      name_description: {
        name_description: 'Nome & descrizione',
      },
      price: {
        price: 'Prezzo',
      },
      type: {
        add: 'Per prima cosa aggiungete un tipo di variazione al vostro prodotto',
        add_another: 'Aggiungere un altro tipo di variazione',
        add_type_name_in_your_own_language:
          'Come minimo, dovreste aggiungere un nome nella vostra lingua.',
        types_variations: 'Tipi e variazioni',
        unnamed: 'Tipo senza nome',
      },
      variation: {
        add: 'Aggiungete una variante al vostro prodotto',
        add_another: 'Aggiungete una nuova variante',
        title: 'Dettagli per',
        unnamed: 'variazione senza nome',
        variation: 'Variazione',
      },
    },
    details: {
      remaining: 'prodotti in magazzino',
      out_of_stock: 'Esaurito',
    },
    management: {
      index: {
        title: 'Prodotti',
      },
      create: {
        category: 'Categoria',
        category_for: 'Categoria per',
        name_and_description: 'Nome e descrizione',
        name_and_description_for: 'Nome e descrizione per',
        price: 'Prezzo',
        price_for: 'Prezzo per',
        tips: {
          category:
            'Il collegamento del prodotto alla categoria corrispondente semplifica la ricerca del prodotto da parte dei visitatori di Sowing.',
          name_and_description:
            'Nominate e descrivete in dettaglio le caratteristiche e i punti di forza del tuo prodotto. La traduzione del nome e della descrizione del vostro prodotto non è obbligatoria, ma aumenta la probabilità che venga notato.',
          price:
            'Siete libero di impostare un prezzo diverso per le variazioni del vostro prodotto.',
          types:
            'Create e raggruppate le diverse varianti del vostro prodotto base.',
          variations: 'Lorem ipsum dolor sit amet',
        },
        title: 'Aggiungete un prodotto',
        unnamed: 'Nuovo prodotto senza nome',
        types: 'Tipi',
        types_for: 'Tipi per',
        variations: 'Variazioni',
        variations_for: 'Variazioni per',
      },
    },
    variations: {
      labels: {
        quantity: 'Quantità',
        selection: 'Selezione',
      },
      select: {
        out_of_stock: 'Esaurito',
      },
    },
  },
}
